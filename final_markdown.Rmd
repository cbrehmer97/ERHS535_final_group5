...
>>>>>>> a03e1550da29dac833502566ba3d1f76c290fef6
...
make_plot <- function(x, y, z){
  ggplot(data = daily_double_year, aes(.data[[x]], .data[[y]], .data[[z]])) +
    geom_tile(color = "white", size = 0.1)
}

make_plot("category_number", "value_number", "number_of_doubles")
...
daily_double_bets <- labeling_positions %>% 
  ...
  group_by(category_number, value_number) %>% 
  summarise(mean = mean(value), minimum = min(value), maximum = max(value))
